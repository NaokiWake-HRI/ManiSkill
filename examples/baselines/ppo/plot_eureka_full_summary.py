"""Outer-loop Eureka full (LLM-only reward generation) summary plot. Works mid-run.

Usage:
    python plot_eureka_full_summary.py                          # latest PushCube run
    python plot_eureka_full_summary.py --env PushCube-v1        # specific env
    python plot_eureka_full_summary.py --run_dir runs/eureka_full/PushCube-v1/...  # specific run
"""
...
OUTER_LOOP_DIR = Path(__file__).parent / "runs" / "eureka_full"
...
def plot_summary(run_dir: Path, out_path: Path | None = None):
    ...
    fig.suptitle(f"Eureka Full - {env_name} ({run_dir.name})",
                 fontsize=12, fontweight="bold")
    plt.tight_layout()

    if out_path is None:
        out_path = run_dir / "eureka_full_summary.png"
...
def main():
    parser = argparse.ArgumentParser(description="Eureka full summary plot")
