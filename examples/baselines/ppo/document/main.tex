...
6-stage humanoid bimanual). Our main findings are:
...
12.5\%, respectively).
(3)~The value of VLM feedback scales with task difficulty and the number of
behavioral stages: multi-stage tasks yield qualitatively new VLM observations
each iteration, while single-stage tasks produce repetitive diagnoses.

These results suggest that VLM-in-the-loop reward design is a powerful tool for
hard manipulation tasks but should be conditionally activated based on current
performance to avoid negative transfer on easy tasks.
\end{abstract}
...
\begin{enumerate}
  ...
  \item Analysis of VLM feedback quality across task difficulty levels,
...
We evaluate on four ManiSkill~\cite{gu2023maniskill2} manipulation tasks:
...
\begin{table}[htbp]
\caption{VLM effectiveness by task difficulty}
...
The value of VLM feedback is strongly correlated with task difficulty, measured
by whether the LLM can solve the task at Iteration~0 without visual feedback.
...
have been permanently lost when VLM-guided candidates underperformed. On hard
tasks, the elite preserves gains between breakthrough iterations.
...
Our results suggest that VLM feedback should be conditionally activated: when
the best candidate already achieves high success rates, VLM feedback risks
over-engineering; when performance is low, VLM provides actionable behavioral
diagnosis. The appropriate threshold is task-dependent and warrants further
investigation. An adaptive system could also terminate the VLM loop when the
same diagnosis repeats for 2+ consecutive iterations.
...
Our main finding is that \textbf{VLM feedback value scales with task
difficulty}:
\begin{itemize}
...
\textbf{visual feedback should be conditionally activated} based on current
task performance level, with early stopping when the VLM has no new
