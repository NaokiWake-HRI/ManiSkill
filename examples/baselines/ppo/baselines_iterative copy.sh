...
for seed in ${seeds[@]}
do
  python ppo_iterative.py --env_id="PickCube-v1" --seed=${seed} \
    --num_envs=512 --num-steps=50 --update_epochs=4 --num_minibatches=32 \
    --total_timesteps=4_000_000 \
    --num_eval_envs=8 \
    --num_segments=10 \
    --exp-name="ppo-iterative-PickCube-v1-state-${seed}" \
    --track \
    ${SKIP_VLM_FLAG}
done
